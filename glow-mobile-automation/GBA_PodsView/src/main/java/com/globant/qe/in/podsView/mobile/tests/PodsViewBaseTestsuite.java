package com.globant.qe.in.podsView.mobile.tests;
...
import com.globant.qe.in.podsView.mobile.listeners.MobilePDFReportGenerator;
...
public class PodsViewBaseTestsuite  extends AbstractTestNGSpringContextTests{
	...
	@Autowired
	PodsViewLoginService loginService;

	@Autowired	
	PodsViewLoginPage loginPage;
	...
	public void springTestContextPrepareTestInstance(ITestContext context) throws Exception {
		...
		mobileDriver = loginPage.getMobileDriver();
		AbstractMobileBasePage.getLogger().log(Status.INFO, "Before Suite started.");
		try {

			loginService = mobileServiceProvider.getPodsViewLoginService().loadPage();
			loginService.signIn();
			
			AbstractMobileBasePage.getLogger().log(Status.INFO, "SignIn Successful.");
			...
			try {
				podsViewHomeService = mobileServiceProvider.getPodsViewHomeService().loadPage();
				AbstractMobileBasePage.getLogger().log(Status.INFO, " SignIn Successful.");
	...
	@BeforeMethod
	public void launchPodsViewApp(Method method) throws Exception {
	...
	public String getTotalExecutionTime() {
		long startTime = loginPage.getExecutionStartTime();
